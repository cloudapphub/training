...
import { ChevronLeft, ChevronRight, BookOpen, Code2, CheckCircle2, Clock, Lightbulb, ArrowLeft, MessageSquare, ChevronDown, ChevronUp, Zap } from "lucide-react";
import { cfnLessons as lessons, cfnInterviewQA } from "./cfnLessons";
      ...
      <p key={i} className="text-slate-300 leading-relaxed text-sm sm:text-base mb-3 last:mb-0">
        {parts.map((part, j) => {
          if (part.startsWith("**") && part.endsWith("**"))
            return <strong key={j} className="text-orange-300 font-semibold">{part.slice(2, -2)}</strong>;
          if (part.startsWith("`") && part.endsWith("`"))
  ...
  return (
    <div className="border border-slate-800 rounded-xl overflow-hidden mb-3">
      ...
      >
        ...
        {open ? <ChevronUp size={16} className="text-orange-400 mt-0.5 shrink-0" /> : <ChevronDown size={16} className="text-slate-500 mt-0.5 shrink-0" />}
      </button>
      {open && (
        <div className="px-5 pb-5 pt-2 bg-slate-950/40">
          ...
          <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2">
            ...
            <p className="text-amber-300 text-xs leading-relaxed">{item.tip}</p>
          </div>
        </div>
      )}
    </div>
...
export default function CloudFormationTraining() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8">
      <div className="w-full px-4 sm:px-10 lg:px-16">
        <Link to="/" className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-orange-400 transition-colors mb-6">
        ...
        <header className="mb-6">
          <div className="flex items-center gap-2 text-orange-400 mb-2">
            <BookOpen size={22} />
            <h1 className="text-2xl sm:text-3xl font-bold">AWS CloudFormation — Complete Training</h1>
          </div>
          <p className="text-slate-400 text-sm">10 deep-dive lessons + Interview Q&A. Covers template syntax, intrinsic functions, Stacks, StackSets, Change Sets, Drift Detection, Custom Resources, and CI/CD best practices.</p>

          {/* Tab switcher */}
          <div className="flex gap-2 mt-5">
            <button onClick={() => setTab("lessons")}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === "lessons" ? "bg-orange-500 text-white" : "bg-slate-800 text-slate-400 hover:text-white"}`}>
              <BookOpen size={15} /> Lessons
            </button>
            <button onClick={() => setTab("qa")}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === "qa" ? "bg-orange-500 text-white" : "bg-slate-800 text-slate-400 hover:text-white"}`}>
              ...
              <div className="flex gap-1.5 items-center flex-wrap justify-center">
                {lessons.map((_, i) => (
                  <button key={i} onClick={() => { setIdx(i); setShowSolution(false); }}
                    className={`w-2.5 h-2.5 rounded-full transition ${i === idx ? "bg-orange-400 w-6" : completed[i] ? "bg-green-500" : "bg-slate-700"}`}
                    aria-label={`Lesson ${i + 1}`} />
                ))}
              </div>
              <button onClick={() => go(1)} disabled={idx === lessons.length - 1} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-orange-500 text-white font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-orange-400">
        ...
        {tab === "qa" && (
          <div>
            <div className="mb-6 p-4 bg-orange-500/10 border border-orange-500/20 rounded-xl">
              <p className="text-orange-300 text-sm font-medium mb-1">AWS DevOps Interview Prep</p>
              <p className="text-slate-400 text-xs">Click any question to reveal the answer and interview tip. Each answer is crafted to demonstrate senior-level understanding.</p>
            </div>
            {cfnInterviewQA.map((section) => (
              <div key={section.category} className="mb-8">
                <h2 className="text-lg font-bold text-orange-400 mb-4 flex items-center gap-2">
                  <span className="w-2 h-5 bg-orange-400 rounded-full inline-block" />
                  {section.category}
                </h2>
