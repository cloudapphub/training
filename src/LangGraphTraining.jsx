...
import { langgraphLessons as lessons } from "./langgraphLessons";
      ...
      <p key={i} className="text-slate-300 leading-relaxed text-sm sm:text-base mb-3 last:mb-0">
        {parts.map((part, j) => {
          if (part.startsWith("**") && part.endsWith("**")) {
            return <strong key={j} className="text-violet-300 font-semibold">{part.slice(2, -2)}</strong>;
          }
          if (part.startsWith("`") && part.endsWith("`")) {
...
export default function LangGraphTraining() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8">
      <div className="w-full px-6 sm:px-10 lg:px-16">
        <Link to="/" className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-violet-400 transition-colors mb-6">
          <ArrowLeft size={16} /> Back to Trainings
        </Link>
        <header className="mb-6">
          <div className="flex items-center gap-2 text-violet-400 mb-2">
            <BookOpen size={22} />
            <h1 className="text-2xl sm:text-3xl font-bold">LangGraph — Build Stateful AI Agents</h1>
          </div>
          <p className="text-slate-400 text-sm">
            10 focused hours. Master StateGraph, state & reducers, conditional routing, tool calling, checkpointing, human-in-the-loop, streaming, error handling, map-reduce, and production deployment.
          </p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 sm:p-7 shadow-xl">
          <div className="flex items-center gap-2 text-xs text-fuchsia-400 uppercase tracking-wider mb-2">
        ...
        <nav className="flex justify-between mt-5">
          ...
          <div className="flex gap-1.5 items-center">
            {lessons.map((_, i) => (
              <button key={i} onClick={() => { setIdx(i); setShowSolution(false); }} className={`w-2.5 h-2.5 rounded-full transition ${i === idx ? "bg-violet-400 w-6" : completed[i] ? "bg-green-500" : "bg-slate-700"}`} aria-label={`Go to lesson ${i + 1}`} />
            ))}
          </div>
          <button onClick={() => go(1)} disabled={idx === lessons.length - 1} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-violet-500 text-white font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-violet-400">
