...
import { javaLessons as lessons } from "./javaLessons";
    ...
    return (
      <p key={i} className="text-slate-300 leading-relaxed text-sm sm:text-base mb-3 last:mb-0">
        {parts.map((part, j) => {
          ...
          if (part.startsWith("`") && part.endsWith("`")) {
            return <code key={j} className="px-1.5 py-0.5 rounded bg-slate-800 text-orange-300 font-mono text-[0.85em]">{part.slice(1, -1)}</code>;
          }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8">
      <div className="w-full px-6 sm:px-10 lg:px-16">
        <Link to="/" className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-amber-400 transition-colors mb-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 sm:p-7 shadow-xl">
          ...
          <section className="mb-5">
            <div className="flex items-center gap-2 text-orange-400 text-sm font-semibold mb-2"><Lightbulb size={16} /> Concept</div>
          ...
          <section className="mb-5">
            <div className="text-rose-400 text-sm font-semibold mb-2">Practice Along</div>
            <p className="text-slate-300 text-sm sm:text-base mb-3">{lesson.practice}</p>
            <button onClick={() => setShowSolution((s) => !s)} className="text-xs px-3 py-1.5 rounded-md bg-rose-500/20 text-rose-300 hover:bg-rose-500/30 border border-rose-500/30">
              {showSolution ? "Hide solution" : "Show solution"}
            </button>
            {showSolution && (
              <pre className="mt-3 bg-black/60 border border-rose-900/50 rounded-lg p-4 overflow-x-auto text-xs sm:text-sm font-mono text-slate-200 whitespace-pre">{lesson.solution}</pre>
            )}
          </section>
        ...
        <nav className="flex justify-between mt-5">
          ...
          <button onClick={() => go(1)} disabled={idx === lessons.length - 1} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-amber-500 text-white font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-amber-400">
