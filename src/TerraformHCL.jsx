...
import { terraformHclLessons as lessons } from "./terraformHclLessons";
...
export default function TerraformHCL() {
      ...
      <div className="w-full px-6 sm:px-10 lg:px-16">
        ...
        <header className="mb-6">
          <div className="flex items-center gap-2 text-yellow-400 mb-2">
            <BookOpen size={22} />
            <h1 className="text-2xl sm:text-3xl font-bold">Terraform HCL Deep Dive & State Management</h1>
          </div>
          <p className="text-slate-400 text-sm">
            8 focused hours. Master HCL syntax, state internals, locking, drift recovery, imports, and production best practices.
          </p>
        ...
        <nav className="flex justify-between mt-5">
          ...
          <div className="flex gap-1.5 items-center">
            {lessons.map((_, i) => (
              <button key={i} onClick={() => { setIdx(i); setShowSolution(false); }} className={`w-2.5 h-2.5 rounded-full transition ${i === idx ? "bg-yellow-400 w-6" : completed[i] ? "bg-green-500" : "bg-slate-700"}`} aria-label={`Go to lesson ${i + 1}`} />
            ))}
          </div>
          <button onClick={() => go(1)} disabled={idx === lessons.length - 1} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-yellow-500 text-slate-900 font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-yellow-400">
