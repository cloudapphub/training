...
import { hsmSwiftLessons as lessons } from "./hsmSwiftLessons";
...
export default function HsmSwiftTraining() {
      ...
      <div className="w-full px-6 sm:px-10 lg:px-16">
        ...
        <header className="mb-6">
          <div className="flex items-center gap-2 text-cyan-400 mb-2">
            <BookOpen size={22} />
            <h1 className="text-2xl sm:text-3xl font-bold">HSM & SWIFT Cross-Border Payments — Deep Dive</h1>
          </div>
          <p className="text-slate-400 text-sm">
            10 focused hours. Master HSM architecture, PKCS#11, XML Digital Signatures, SWIFT infrastructure, and cryptographic security for cross-border payments.
          </p>
          <div className="mt-4 h-2 bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-cyan-400 to-teal-500 transition-all duration-300" style={{ width: `${progress}%` }} />
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 sm:p-7 shadow-xl">
          <div className="flex items-center gap-2 text-xs text-teal-400 uppercase tracking-wider mb-2">
        ...
        <nav className="flex justify-between mt-5">
          ...
          <button onClick={() => go(1)} disabled={idx === lessons.length - 1} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-cyan-500 text-slate-900 font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-cyan-400">
